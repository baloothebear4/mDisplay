...
class Infos(object):
	...
	def __init__(self):
		...
		self.songcoverart = AirplayCoverArt()		# CoverArt, (with bytes, base64, mime and stuff)
		...
		self.progress = None    # shairport metadata indicating progress through the song
		self.type     = None    # ShairportSync metadata code
		self.elapsed  = None
		self.elapsedpc = None
		self.duration  = None
...
class AirplayCoverArt(object):
	...
	def base64(self):
		if self._base64:
			print("AirplayCoverArt: base 64 - created size %d" % len(self._base64))
			return self._base64
		if self._binary:
			print("AirplayCoverArt: base 64: binary - created size %d" % len(self._binary))
	...
	def binary(self):
		if self._binary:
			print("AirplayCoverArt: binary - created size %d" % len(self._binary))
			return self._binary
	...
	def mime(self):
		...
		if self.binary:
			...
			print("METHOD DEPRECATED to compile")
...
class Item(object):
	def __init__(self, e):
		if isinstance(e, str):
			...
			# except xml.ParseError:
				...
				logger.warn("Skipping malformed line: [{line}] Error {err}".format(line=e, err=err), exc_info=True)
				return
	...
	@property
	def data_date(self):
		return datetime.fromtimestamp(self.data_int/1000)
